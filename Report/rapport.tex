...
\AXCm{}
\LLm{}
\UICm{\bigcup_{\delta\in Im(\Delta)} \delta\#name.Dom(\delta\#\pi) \subset \Po_T}
\end{prooftree}
\vspace{0cm}
...
\AXCm{}
\LLm{}
\UICm{\mathfrak{P}_F(\I)\times \{P,R\} \subset \Po_T}
\end{prooftree}
\vspace{0cm}
...
Once we have defined $\I$, we need to define a subtype relation on it, coinductively again. Th sub typing relation does not take into account the role of the port. That is why we use the wild-card character \_.
...
\AXCm{ret \preceq ret'}
\LLm{}
\BICm{<(a_1,...,a_n),ret> \preceq <(a'_1,...,a'_n),ret'>}
\end{prooftree}
\vspace{0cm}
...
\doubleLine
\AXCm{
getType(descr.port) \preceq b
}
\LLm{}
\UICm{descr.port \preceq b}
\end{prooftree}
\vspace{0cm}
...
\doubleLine
\AXCm{
a \preceq getType(descr.port)
}
\LLm{}
\UICm{a\preceq descr.port}
...
\AXCm{
\begin{array}{c} 
\tau_1 \preceq \upsilon_1\\
\vdots\\
\tau_n \preceq \upsilon_n\\
\end{array}
}

\LLm{}
\UICm{<\{serv_1 : \tau_1 ; ...; serv_n : \tau_n\}, _> \preceq <\{serv_1 : \upsilon_1 ; ...; serv_n : \upsilon_n; ...; serv_p : : \upsilon_p\},_ >}
...
\AXCm{}
\LLm{Var_{loc}}
\UICm{\TypeEnvs{} \vdash v : \Theta(v); \TypeEnvswst{}}
\end{prooftree}
\vspace{0cm}
...
\LLm{Var_{descr}}
\RLm{v\notin Dom(\Theta)}
\UICm{\TypeEnvs{} \vdash v : \Gamma(\sigma)(v);\TypeEnvswst{}}
\end{prooftree}
\vspace{0cm}

\begin{prooftree}
\AXCm{\begin{array}{c}
\TypeEnvs{}\vdash expr : T ; \TypeEnvswst{'}\\
\TypeEnvs{'} \vdash arg_1 : T_1 ; \TypeEnvswst{_1}\\
\vdots\\
\TypeEnvs{_{n-1}} \vdash arg_n : T_n ; \TypeEnvswst{_n}\\
\end{array}}

\AXCm{T\#type(serv) = <(T_1,...T_n),T_{ret}>}
\LLm{Call}
\BICm{\TypeEnvs{}\vdash expr.serv(arg_1,...arg_n) : T_{ret};\TypeEnvswst{_n}}
\end{prooftree}
\vspace{0cm}


\begin{prooftree}
\AXCm{\TypeEnvs{}\vdash expr : T;\TypeEnvswst{'}}
\LLm{At}
\UICm{\TypeEnvs{}\vdash port\textbf{@}expr :T';\TypeEnvswst{'}}
\end{prooftree}
\hspace{0.2cm} where $T' = \Gamma(findProv(T)\#self_c)(port)$
...
\AXCm{\TypeEnvs{}\vdash expr : T;\TypeEnvswst{'}}
\LLm{Path}
\UICm{\TypeEnvs{}\vdash_{e}~!expr : findProv(T); \TypeEnvswst{'}}
\end{prooftree}
\vspace{0cm}


\begin{prooftree}
\AXCm{\TypeEnvswsPers{'}{}{},max \vdash arch \implies ();\TypeEnvswst{''}}
\LLm{New}
\UICm{\TypeEnvs{}\vdash descr.new() \implies val;\TypeEnvswst{''}}
\end{prooftree}
\hspace{0.2cm} with : 
\begin{itemize}
\item $newCompo = <[\![descr\#\pi_P]\!]_{pp},[\![descr\#\pi_{R}]\!]_{rp},>$
...
\caption{Type system of \compo{} expressions}
...
\begin{prooftree}
\AXCm{\TypeEnvs{}\vdash_{p} prog1 : T ; \TypeEnvsws{'}}
\AXCm{\TypeEnvs{'}\vdash_{p} prog2 : U ; \TypeEnvsws{''}}
\LLm{Seq}
\BICm{\TypeEnvs{}\vdash_{p} prog1;prog2 : U; \TypeEnvsws{''}}
\end{prooftree}
\vspace{0cm}
...
\AXCm{\TypeEnvs{}\vdash expr : T; \TypeEnvswst{'}}
\LLm{Ret}
\UICm{\TypeEnvs{}\vdash \textbf{return}~expr : T; \TypeEnvsws{'}}
\end{prooftree}
+connect $\Omega$
\vspace{0cm}
...
\AXCm{}
\LLm{Require}
\UICm{\TypeEnvs{}\vdash \textbf{require}~name:T : ();\TypeEnvswsPers{}{+ [name\rightarrow <type,\sigma, empty, 0>]}{}}
\end{prooftree}
\vspace{0cm}
...
\begin{prooftree}
\AXCm{\TypeEnvs{}\vdash_{e} expr_1 : T_1; \TypeEnvswst{'}}
\AXCm{\TypeEnvs{'}\vdash_{e} expr_2 : T_2; \TypeEnvswst{''}}

\LLm{Connect}
\BICm{\TypeEnvs{}\vdash_{p} \textbf{connect}~expr_1~\textbf{to}~ expr_2 : ();\TypeEnvsws{_{res}}}

\end{prooftree}
\hspace{0.2cm} with :
\begin{itemize}
\item $T_1\#interface \subset T_2\#interface $

\item
 $\begin{array}{l}(\TypeEnvsws{_{res}}) =\\
\qquad 
 \begin{cases}
...
\Gamma(pgp\#self_c)\#\pi_X(port)&\text{if it is defined and if $port\in pgp\#ap$} \\
\perp & \text{else}
\end{cases}
\]
